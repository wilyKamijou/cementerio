{{-- resources/views/admin/inhumaciones/modals/inhumacion-crear.blade.php --}}
<div class="modal fade" id="modalCrearInhumacion" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-plus-circle"></i> Nueva Inhumación
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>

            <form id="formCrearInhumacion" method="POST">
                @csrf
                <div class="modal-body">
                    <!-- DATOS DEL FALLECIDO - 4 COLUMNAS -->
                    <h6 class="mb-3"><i class="fas fa-user"></i> Datos del Fallecido</h6>

                    <div class="row">
                        <div class="col-md-3">
                            <div class="mb-3">
                                <label>Nombre <span class="text-danger">*</span></label>
                                <input type="text" name="nombre" class="form-control"
                                    placeholder="Nombre del fallecido" required>
                                <small class="form-text">Nombre(s) del fallecido</small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="mb-3">
                                <label>Apellido Paterno <span class="text-danger">*</span></label>
                                <input type="text" name="paterno" class="form-control" placeholder="Apellido paterno"
                                    required>
                                <small class="form-text">Primer apellido</small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="mb-3">
                                <label>Apellido Materno</label>
                                <input type="text" name="materno" class="form-control"
                                    placeholder="Apellido materno">
                                <small class="form-text">Segundo apellido (opcional)</small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="mb-3">
                                <label>Causa de Muerte</label>
                                <input type="text" name="causaMuer" class="form-control"
                                    placeholder="Causa de muerte">
                                <small class="form-text">Causa del fallecimiento</small>
                            </div>
                        </div>
                    </div>

                    <!-- FECHAS IMPORTANTES - 3 COLUMNAS -->
                    <h6 class="mb-3 mt-3"><i class="fas fa-calendar-alt"></i> Fechas Importantes</h6>

                    <div class="row">
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label>Fecha de Nacimiento</label>
                                <input type="date" name="fechaNaci" class="form-control">
                                <small class="form-text">Fecha de nacimiento (opcional)</small>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label>Fecha de Defunción <span class="text-danger">*</span></label>
                                <input type="date" name="fechaDefun" class="form-control" required>
                                <small class="form-text">Fecha de fallecimiento</small>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label>Fecha de Inhumación <span class="text-danger">*</span></label>
                                <input type="date" name="fechaInhuma" class="form-control" required>
                                <small class="form-text">Fecha del entierro</small>
                            </div>
                        </div>
                    </div>

                    <!-- DATOS DE LA INHUMACIÓN - 2 COLUMNAS -->
                    <h6 class="mb-3 mt-3"><i class="fas fa-dove"></i> Datos de la Inhumación</h6>

                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label>Espacio <span class="text-danger">*</span></label>
                                <select name="idEspacio" class="form-control" required>
                                    <option value="">-- Seleccionar espacio --</option>
                                    @foreach ($espaciosDisponibles as $espacio)
                                        <option value="{{ $espacio->idEspacio }}">
                                            @if ($espacio->direccion)
                                                Espacio #{{ $espacio->idEspacio }} -
                                                Secc: {{ $espacio->direccion->seccion }} |
                                                Núm: {{ $espacio->direccion->numero }} |
                                                Calle: {{ $espacio->direccion->calle }} |
                                                Fila: {{ $espacio->direccion->fila }}
                                            @else
                                                Espacio #{{ $espacio->idEspacio }} - Sin dirección
                                            @endif
                                        </option>
                                    @endforeach
                                </select>
                                <small class="form-text">Seleccione el espacio funerario (solo disponibles)</small>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label>Tipo de Inhumación <span class="text-danger">*</span></label>
                                <select name="idTipo" class="form-control" required>
                                    <option value="">-- Seleccionar tipo --</option>
                                    @foreach ($tiposInhumacion as $tipo)
                                        <option value="{{ $tipo->idTipo }}">
                                            {{ $tipo->nombre }} - ${{ number_format($tipo->precio, 2) }}
                                        </option>
                                    @endforeach
                                </select>
                                <small class="form-text">Tipo de servicio funerario</small>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                        <i class="fas fa-times"></i> Cancelar
                    </button>
                    <button type="submit" class="btn btn-success">
                        <i class="fas fa-save"></i> Guardar
                    </button>
                </div>
            </form>
        </div>
    </div>
</div>
